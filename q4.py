...
class QNetwork():
    ...
    def __init__(self, feature_shape, num_actions):
        ...
        self.features_shape = feature_shape
        self.num_actions = num_actions
        input = Input(shape=feature_shape)
        v = self.func_v(input)
        a = self.func_a(input)
        a_avg = Lambda(lambda x: K.mean(x))(a)
        a_avg_rep = RepeatVector(self.num_actions)(a_avg)
        out = add([v, a, -a_avg_rep])
        self.model = Model(inputs=[input], outputs=[out])
        self.model.compile(optimizer=keras.optimizers.Adam(), loss=keras.losses.mse)


    def func_v(self, input):
        v = Dense(36, activation='relu')(input)
        v = Dense(96, activation='relu')(v)
        v = Dense(1)(v)
        v = RepeatVector(self.num_actions)(v)
        return v

    def func_a(self, input):
        a = Dense(36, activation='relu')(input)
        a = Dense(96, activation='relu')(a)
