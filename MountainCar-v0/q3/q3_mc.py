...
class DQN_Agent():
    ...
    def train(self):
        ...
        for i_episode in range(self.max_episode):
            ...
            avg50 = self.latest_k(all_reward)
            print('----- Episode {} done with {} steps, reward={}, avg50={:.4f}, avg={:.4f} -----'
                  .format(i_episode, curr_iteration, curr_reward, avg50, np.average(all_reward)))
            if i_episode % 50 == 0:
                model.save("./model/e{}_avg50_{:.4f}_avg_{}.h5"
                           .format(i_episode, avg50, np.average(all_reward)))

        np.save('reward_summary.npy', reward_summary)
        model.save("./model/e{}_avg50_{:.4f}_avg_{:4f}.h5"
                   .format(self.max_episode, self.latest_k(all_reward), np.average(all_reward)))


    def latest_k(self, arr, k=50):
